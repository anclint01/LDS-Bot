...
bot.on('message', message => {
  ...
  switch(command.trim()){
    ...
    break;
    case "flip":
      var a = 2 * Math.random() + 1, b; b = 2 > a ? "The coin landed on: **heads**":"The coin landed on: **tails**"; message.reply(b);
    break;
    case "8ball":
    ...
    collection.forEach(function(key,id){
      ...
      array.sort(function(a,b){return a[2]-b[2]; });
    ...
    case "createrole":
     if(!message.member.roles.has(modRole.id)) {
          ...
          message.guild.createRole({ name: result})
    ...
    case "eval":
      ...
      message.channel.sendEmbed(embed).then(function(){message.delete();}); 
      }
...
//bot.login("MzAzNzQ1MjExNDAwNDU0MTQ0.C9cmvw.zXpUlBRfa7Juw6mnEkLt4B0ZIuI");
