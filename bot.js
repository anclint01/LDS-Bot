...
bot.on("message", message => {
            ...
            if (startListening) {
                if (message.author.id === playerID) {
                    if (message.content.toLowerCase() === "exit") {
                        message.channel.send("Successfuly exited the game").then(msg => {
                            msg.delete(1000);
                            message.delete(1000);
                        })
            ...
            function setUpGame(guild, channel) {
                ...
                message.channel.send("To play, merely type a letter you think is in the word. During the game, every message you send will be treated as if you are playing and so will be deleted, if you wish at any time to abort, please type ``exit``").then(msg => {
                    msg.delete(10000)
                });
            }

            function rep(str, pos, content) {
                var str = str;
                str = setCharAt(str, pos, content);
                wordHint = str;
            }
            ...
            var thing = {
                embed: {
                    author: {
                        name: wordHint.split("").join(" ")
                    },
                    description: hangmanIterations[wrongGuessesNumber].replace(/\t/g, ""),
                    fields: [{
                            name: "Wrong Letters",
                            value: ((wrongGuessesNumber <= 0) ? "⠀" : (wrongGuessesNumber === 1) ? wrongGuesses[0] : wrongGuesses.join("")),
                            inline: true
                        },
                        {
                            name: "‎ ",
