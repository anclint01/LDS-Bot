...
import { ChatInputCommandInteraction, GuildMember, ComponentType, InteractionCollector, Message, ActionRowBuilder, StringSelectMenuBuilder, StringSelectMenuInteraction, JSONEncodable, APIEmbed, User, SelectMenuInteraction, ButtonBuilder } from 'discord.js'
import BaseCommand from '../../../types/command'
import {gc_talks} from '../../../../gc.json'
import { delete_button, getClient, get_result } from '../../../constants/func';
...
class Command extends BaseCommand {
  ...
  async execute_slash (interaction: ChatInputCommandInteraction ) {  
    ...
    prms.then(async () => {
      ...
      const collector: InteractionCollector<StringSelectMenuInteraction> = interaction.channel?.createMessageComponentCollector({ filter, componentType: ComponentType.StringSelect, time: 120000, max: 1 })!;
      collector.on("collect", async (i: StringSelectMenuInteraction) => {
        ...
        let result = await get_result(title + " by " + speaker + ", " + c_month + ", " + c_year, construct_link, "General Conference")

        if (result === undefined) {
          await i.reply("Uh oh, something went wrong");
          collector.stop();
        }
        ...
        if (typeof result === "object") {
          if (Array.isArray(result)) {
            ...
            new Paginator(interaction.user as User, result[0] as APIEmbed, result, false, undefined, i as StringSelectMenuInteraction)._new()
            await interaction.editReply({content: "Search sent", components: []});
            collector.stop();
          } else {
            result = result as APIEmbed;
            message = await i.reply({ embeds:[result], fetchReply: true});
            await interaction.editReply({content: "Search sent", components: []});
            collector.stop();
          }
        }
      })
      collector.on("end", async (reason: string, i) => {
        if (reason === "time") {
          await interaction.editReply({content: "Search timed out."})
          let deleteButton = await delete_button((<Message>message).member as GuildMember);
  ...
  execute (message: Message, args: string[]) {
    ...
    prms.then(async () => {
      ...
      const filter = (i: StringSelectMenuInteraction) => i.customId === 'search' && i.isStringSelectMenu() && i.user.id == message.author.id && i.message.id == m.id;
      const collector: InteractionCollector<StringSelectMenuInteraction> = m.channel.createMessageComponentCollector({ filter, componentType: ComponentType.StringSelect, time: 120000 , max: 1 });
      collector.on("collect", async (i: StringSelectMenuInteraction) => {
        let choice: number = parseInt(i.values[0]);
