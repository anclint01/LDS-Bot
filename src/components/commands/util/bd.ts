...
class Command extends BaseCommand {
  ...
  execute_slash (interaction: CommandInteraction) {
    ...
    prms.then(async () => { 
      if (result === "") {
        return interaction.reply({content: "I couldn't find a Bible Dictionary entry with that name", ephemeral: true}).catch((err: any) => {
      ...
      request("https://www.churchofjesuschrist.org/" + result, function (error: any, response: any, body: any) {
        if (error) {
          ...
          let final = data.replace(/<\/?(?:div|small|span|nav|ul|li).*?>/gmis, "");
            ...
            if (final.match(/\r?\n/)) {
              ...
              for (let i = 0; i < splits.length; i++) {
                ...
                if (new_s.length <= 2500) des_s = new_s; 
                  ...
                  page_array.push({
                    ...
                    description: des_s,
                    footer: {
                      text: "LDS-Bot | Bible Dictionary (churchofjesuschrist.org)",
              ...
              if (des_s.length > 0) {
                page_array.push({
                  ...
                  description: des_s,
                  footer: {
                      text: "LDS | Bible Dictionary (churchofjesuschrist.org)",
                      icon_url: getClient().user?.displayAvatarURL()
                  }
                })                
              }              
  ...
  execute (message: Message, args: string[]) {
    ...
    prms.then(async () => { 
      ...
      if (result === "") {
        return message.channel.send({content: "I couldn't find a Bible Dictionary entry with that name"}).catch((err: any) => {
