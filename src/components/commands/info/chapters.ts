import { APIEmbed, ChatInputCommandInteraction, Embed, TextChannel, User } from "discord.js";
...
class Command extends BaseCommand {
  ...
  async execute_slash (interaction: ChatInputCommandInteraction) {  
    ...
    let requested_book: MainBook = (from == "bom" ? bom : (from == "pgp" ? pgp : (from == "d&c" ? dc : (from == "nt" ? nt : ot)))) as MainBook;
    ...
    let page_array: APIEmbed[] = [];
    for (let i = 0; i < book_chapters.length; i++) {
      if (description.length >= 2000) {
        LDSBot_Embed_Push(page_array, "Chapters in " + requested_sub_book.book, description)
        description = "";
      }  
      description += `Chapter ${i+1} - \`${book_chapters[i].verses?.length} verses\`\n`
    }
    if (description.length > 0) LDSBot_Embed_Push(page_array, "Chapters in " + requested_sub_book.book, description)
    ...
    if (page_array.length < 2) {
      ...
      new Paginator(interaction.user as User, page_array[0], page_array, false, undefined, interaction)._new();
    }    
  }
  ...
  async execute (message: Message, args: string[]) {
    ...
    } else new Paginator(message.author as User, page_array[0], page_array, false, message as Message)._new();
  }
}
